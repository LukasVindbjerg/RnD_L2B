...
train_encodings = tokenizer(train_texts_with_concat, padding=True, truncation=True, return_tensors="pt")
...
training_args = TrainingArguments(
    output_dir=f'./results/results_label_concat_{timestamp}',  # Directory to save model checkpoints
    num_train_epochs=epochs,
    per_device_train_batch_size=batch_size,
    per_device_eval_batch_size=batch_size,  # Same as training batch size
    warmup_steps=warmup_steps,
    weight_decay=0.01,
    logging_dir=f'./logs/label_concat_{timestamp}',  # Directory to save the logs
...
model.save_pretrained(f'./models/t5_label_concat_model_{timestamp}')
tokenizer.save_pretrained(f'./models/t5_label_concat_tokenizer_{timestamp}')
